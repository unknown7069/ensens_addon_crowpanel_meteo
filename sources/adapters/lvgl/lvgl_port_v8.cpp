...
#include "lvgl_port_v8.h" //adapters/lvgl/
...
__attribute__((always_inline)) static inline void copy_pixel_8bpp(uint8_t* to, const uint8_t* from)
{
    *to++ = *from++;
}
        ...
        {                                                                                          \
            ...
            {                                                                                      \
                ...
                {                                                                                  \
                    ...
                    {                                                                              \
                        ((uint16_t*)to)[(mirrored_y)*h + x] =                                      \
                            *((uint32_t*)(from_next + y)) & 0xFFFF;                                \
                        ((uint16_t*)to)[(mirrored_y - 1) * h + x] =                                \
                            (*((uint32_t*)(from_next + y)) >> 16) & 0xFFFF;                        \
                        ((uint16_t*)to)[(mirrored_y - 2) * h + x] =                                \
                            *((uint32_t*)(from_next + y + 2)) & 0xFFFF;                            \
        ...
        {                                                                                          \
            ...
            {                                                                                      \
                ...
                {                                                                                  \
                    ...
                    {                                                                              \
                        ((uint16_t*)to)[y * h + (h - 1 - x)] =                                     \
                            *((uint32_t*)(from_next + y)) & 0xFFFF;                                \
                        ((uint16_t*)to)[(y + 1) * h + (h - 1 - x)] =                               \
                            (*((uint32_t*)(from_next + y)) >> 16) & 0xFFFF;                        \
                        ((uint16_t*)to)[(y + 2) * h + (h - 1 - x)] =                               \
                            *((uint32_t*)(from_next + y + 2)) & 0xFFFF;                            \
...
__attribute__((always_inline)) IRAM_ATTR static inline void
rotate_copy_pixel(const uint8_t* from, uint8_t* to, uint16_t x_start, uint16_t y_start,
                  uint16_t x_end, uint16_t y_end, uint16_t w, uint16_t h, uint16_t rotate)
{
    ...
    // ESP_LOGI(TAG, "rotate: end, time used:%d", (int)(esp_log_timestamp() -
    // time));
}
 ...
 *
 * @note This function is used to avoid tearing effect, and only work with LVGL
 * direct-mode.
 */
...
static void flush_dirty_copy(void* dst, void* src, lv_port_dirty_area_t* dirty_area)
{
    ...
    for (int i = 0; i < dirty_area->inv_p; i++)
    {
        ...
        if (dirty_area->inv_area_joined[i] == 0)
        {
            ...
            rotate_copy_pixel((uint8_t*)src, (uint8_t*)dst, x_start, y_start, x_end, y_end,
...
static void flush_callback(lv_disp_drv_t* drv, const lv_area_t* area, lv_color_t* color_map)
{
    ...
    if (lv_disp_flush_is_last(drv))
    {
        ...
        if (drv->full_refresh)
        {
            ...
            rotate_copy_pixel((uint8_t*)color_map, (uint8_t*)next_fb, offsetx1, offsety1, offsetx2,
                              offsety2, LV_HOR_RES, LV_VER_RES, LVGL_PORT_ROTATION_DEGREE);
...
void flush_callback(lv_disp_drv_t* drv, const lv_area_t* area, lv_color_t* color_map)
{
    ...
    lcd->drawBitmap(offsetx1, offsety1, offsetx2 - offsetx1 + 1, offsety2 - offsety1 + 1,
...
static void update_callback(lv_disp_drv_t* drv)
{
    ...
    ESP_UTILS_LOGD("Current mirror x: %d, mirror y: %d, swap xy: %d", disp_init_mirror_x,
                   disp_init_mirror_y, disp_init_swap_xy);
#endif
}
...
static lv_disp_t* display_init(LCD* lcd)
{
    ...
    {
        lvgl_buf[i] =
            heap_caps_malloc(buffer_size * sizeof(lv_color_t), LVGL_PORT_BUFFER_MALLOC_CAPS);
        assert(lvgl_buf[i]);
        ESP_UTILS_LOGD("Buffer[%d] address: %p, size: %d", i, lvgl_buf[i],
                       buffer_size * sizeof(lv_color_t));
    }
#else
    // To avoid the tearing effect, we should use at least two frame buffers:
    // one for LVGL rendering and another for LCD refresh
    buffer_size = lcd_width * lcd_height;
#if (LVGL_PORT_DISP_BUFFER_NUM >= 3) && (LVGL_PORT_ROTATION_DEGREE == 0) && LVGL_PORT_FULL_REFRESH

    // With the usage of three buffers and full-refresh, we always have one
    // buffer available for rendering, eliminating the need to wait for the
    // LCD's sync signal
    ...
    if (lcd->getBasicAttributes().basic_bus_spec.isFunctionValid(
            LCD::BasicBusSpecification::FUNC_SWAP_XY) &&
        lcd->getBasicAttributes().basic_bus_spec.isFunctionValid(
            LCD::BasicBusSpecification::FUNC_MIRROR_X) &&
        lcd->getBasicAttributes().basic_bus_spec.isFunctionValid(
            LCD::BasicBusSpecification::FUNC_MIRROR_Y))
    {
...
static bool tick_init(void)
{
    ...
    const esp_timer_create_args_t lvgl_tick_timer_args = { .callback = &tick_increment,
                                                           .name     = "LVGL tick" };
    ESP_UTILS_CHECK_ERROR_RETURN(esp_timer_create(&lvgl_tick_timer_args, &lvgl_tick_timer), false,
                                 "Create LVGL tick timer failed");
    ESP_UTILS_CHECK_ERROR_RETURN(esp_timer_start_periodic(lvgl_tick_timer,
                                                          LVGL_PORT_TICK_PERIOD_MS * 1000),
                                 false, "Start LVGL tick timer failed");
...
{
    ESP_UTILS_CHECK_ERROR_RETURN(esp_timer_stop(lvgl_tick_timer), false,
                                 "Stop LVGL tick timer failed");
    ESP_UTILS_CHECK_ERROR_RETURN(esp_timer_delete(lvgl_tick_timer), false,
                                 "Delete LVGL tick timer failed");
    return true;
}
#endif
